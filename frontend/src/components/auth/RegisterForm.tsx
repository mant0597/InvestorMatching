  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 2) {
      ...
      try {
        ...
        // If uploading a PDF (startup with pdfFile), send multipart/form-data (multer expects 'pitchDeck')
        if (role === 'startup' && pdfFile) {
          const fd = new FormData();
          fd.append('name', companyName.trim());
          fd.append('email', companyEmail.trim());
          fd.append('password', password);
          fd.append('role', 'startup');

          // startup fields - append as individual fields (multer will populate req.body)
          fd.append('companyName', companyName);
          fd.append('category', category);
          fd.append('foundingYear', foundingYear);
          fd.append('founderName', founderName);
          fd.append('fundingTaken', String(fundingTaken));
          fd.append('fundingRound', fundingTaken ? fundingRound : '');
          fd.append('description', description || '');
          fd.append('profileCompleted', 'true');

          // attach file field name 'pitchDeck' (must match multer.single('pitchDeck'))
          fd.append('pitchDeck', pdfFile);

          const res = await fetch(`${API_BASE}/api/auth/register`, {
            method: 'POST',
            credentials: 'include',
            body: fd // browser sets Content-Type boundary automatically
          });
          ...
          // navigate to dashboard and allow AuthProvider to pick up /me
          navigate('/dashboard');
          return;
        }

        // otherwise send JSON via existing AuthContext.register (keeps backward compatibility)
        const registerPayload: any = {
          name: role === 'startup' ? companyName : investorName,
          email: role === 'startup' ? companyEmail : investorEmail,
          password,
        ...
        if (role === 'startup') {
          registerPayload.profileData = {
            ...
            fundingRound: fundingTaken ? fundingRound : null,
            description,
            pitchDeckFile: pdfFile ? pdfFile.name : null,
            profileCompleted: true
          };
        } else if (role === 'investor') {
          registerPayload.profileData = {
        ...
        await register(registerPayload);
        // AuthContext.register should handle token/store/navigation
      } catch (err: any) {
        const msg = err?.body?.message || err?.message || 'Failed to create an account. Please try again.';
        setError(msg);
      } finally {
